...
# 
# sigJunctions[, is_neand := variant_id %in% neand_list]

junction <- sigJunctions[variant_id == "15_86220629_T_C_b37" & start == 86283503 & end == 86284277]
sigJunction = junction$intron_cluster
cluster_to_plot <- "clu_7657_NA"
main_title = ""
...
make_sQTL_cluster_plot <- function(
  ...
  counts$cluster_id <- str_split_fixed(rownames(counts), ":", 4)[, 4]
  y <- t(counts[counts$cluster_id == cluster_to_plot,]) %>% as.data.frame()
  y <- y[-nrow(y),]
  keep_names <- rownames(y)
  y <- sapply(y, as.integer)
  rownames(y) <- keep_names
  x <- meta$group
  ...
  intron_meta$verdict <- introns_to_plot$verdict[match(paste0(intron_meta$chr,":",intron_meta$start,"-",intron_meta$end), introns_to_plot$coord)]
